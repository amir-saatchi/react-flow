import React from "react";

interface UsbIconProps {
  className?: string;
}

const UsbIcon: React.FC<UsbIconProps> = ({ className }) => {
  return (
    <svg
      className={`w-6 h-6 ${className}`}
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M11.7773 18.6926V2.78711M11.7773 2.78711L9.65715 4.82628M11.7773 2.78711L13.8974 4.82628M12.5134 15.6026L15.7246 14.9849C17.175 14.7059 18.2241 13.4283 18.2241 11.9409M11.5616 12.8572L8.35046 12.2395M8.35046 12.2395C6.90004 11.9605 5.85095 10.6829 5.85095 9.19548C6.16163 11.0569 6.62649 11.8047 8.35046 12.2395Z"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M4.53241 6.76698C5.26829 6.04642 6.43379 6.04642 7.16967 6.76698C7.92122 7.50287 7.92122 8.72447 7.16967 9.46036C6.43379 10.1809 5.26829 10.1809 4.53241 9.46036C3.78086 8.72447 3.78086 7.50287 4.53241 6.76698Z"
        fill="currentColor"
      />
      <path
        d="M10.4063 18.7771C11.1422 18.0566 12.3077 18.0566 13.0436 18.7771C13.7951 19.513 13.7951 20.7346 13.0436 21.4705C12.3077 22.191 11.1422 22.191 10.4063 21.4705C9.65479 20.7346 9.65479 19.513 10.4063 18.7771Z"
        fill="currentColor"
      />
      <path
        d="M18.9208 8.89416H17.527C16.9137 8.89416 16.4166 9.3913 16.4166 10.0046V11.4753C16.4166 12.0886 16.9137 12.5858 17.527 12.5858H18.9208C19.5341 12.5858 20.0312 12.0886 20.0312 11.4753V10.0046C20.0312 9.3913 19.5341 8.89416 18.9208 8.89416Z"
        fill="currentColor"
      />
    </svg>
  );
};

export default UsbIcon;
